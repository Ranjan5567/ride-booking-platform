...
Initially, we considered splitting by microservices - some services on AWS, some on GCP. But that felt forced. We asked ourselves: "What if we split by *purpose* instead of by service?"
...
The biggest challenge was cross-cloud communication. We initially thought about using VPNs or direct network connections, but that's complex and expensive. Then we discovered Pub/Sub - it's designed exactly for this use case. It handles authentication, retries, and scaling automatically. This was a game-changer.
...
- No service discovery (hardcode IPs? DNS? Yuck.)
...
We considered it, but Kubernetes felt like the better long-term choice.
...
2. **Auto-Scaling**: This was a requirement, and Kubernetes HPA (Horizontal Pod Autoscaler) is battle-tested. We set a CPU threshold (5%), and Kubernetes automatically adds/removes pods. During load testing, we watched pods scale from 2 to 10 in minutes. Magic.

3. **Service Discovery**: Kubernetes DNS is brilliant. Services can find each other by name (`http://payment-service:8004`). No hardcoded IPs, no manual DNS configuration. It just works.
...
Kubernetes felt like overkill at first, but it paid off. The auto-scaling, service discovery, and GitOps integration made our lives easier. Plus, it's a valuable skill - Kubernetes is everywhere in the industry. Learning it was worth the investment.
...
1. **Managed Service**: This was huge. Pub/Sub is fully managed - no servers, no configuration, no maintenance. We create a topic, publish messages, and consume them. That's it.

2. **Cross-Cloud**: This was the key requirement. Pub/Sub is a GCP service, but it has SDKs for all languages and can be accessed from anywhere (with proper authentication). Ride Service (on AWS) publishes to Pub/Sub (on GCP) using the Python SDK. It just works.
...
Pub/Sub was the perfect solution for cross-cloud event streaming. It's managed, scalable, reliable, and cost-effective. The authentication setup was a bit complex, but once configured, it just works. For connecting AWS and GCP services, Pub/Sub is the way to go.
...
2. **Windowing**: Flink's windowing API is beautiful. We can define tumbling windows (60 seconds), sliding windows, or session windows with just a few lines of code. The windowing logic is built-in and optimized. We don't have to implement it ourselves.
...
1. **Serverless**: This is the key benefit. No servers to manage. No containers to deploy. No scaling to configure. We write a function, deploy it, and it just works. When a notification is needed, Lambda runs the function. When it's not needed, Lambda doesn't cost anything.
...
3. **Auto-Scaling**: This is magical. If we get a sudden spike (1000 notifications in a minute), Lambda automatically spins up more instances. No configuration needed. It just works.

4. **Requirement Compliance**: We needed to demonstrate serverless architecture. Lambda is the poster child for serverless. It's a perfect example.
...
That's it. 5 lines of code. In a real system, this would send emails, SMS, or push notifications. For the demo, we just log to CloudWatch.
...
But then:
- **Reproducibility**: If we need to recreate the infrastructure (for testing, disaster recovery, or a new environment), we'd have to remember every step. Good luck with that.
...
2. **Kubernetes Integration**: Prometheus integrates seamlessly with Kubernetes:
   ...
   This "just works" - no complex configuration needed.

3. **Rich Dashboards**: Grafana is the visualization layer. It's beautiful and powerful. We can create dashboards showing:
   - Pod count over time
   - CPU and memory usage
   - Request rate and latency
   - Error rates

   During load testing, watching the dashboards update in real-time is satisfying. You can see the system respond to load.
